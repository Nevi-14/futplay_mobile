...
export class ReservacionesService {
  ...
  syncGetDisponibilidadReservaciones(Cod_Cancha, Hora_Inicio, Hora_Fin) {
    return this.getDisponibilidadReservaciones(
      Cod_Cancha,
      Hora_Inicio,
      Hora_Fin
    ).toPromise();
  }
  syncGetReservacionesCanchaFechaToPromise(
    Cod_Cancha,
    Fecha_Inicio,
    Fecha_Fin
  ) {
    return this.getReservacionesCanchaFecha(
      Cod_Cancha,
      Fecha_Inicio,
      Fecha_Fin
    ).toPromise();
  ...
  async cargarReservaciones() {
    if (this.usuariosService.usuarioActual) {
      return this.syncgGtReservacionesConfirmadas(
        this.usuariosService.usuarioActual.Cod_Usuario
      ).then((reservaciones2) => {
        this.retosConfirmados = reservaciones2;

        return this.syncGetReservacionesAbiertasToPromise().then(
          (reservaciones) => {
            this.retosAbiertos = reservaciones;
            return this.syncgGtReservacionesRecibidas(
              this.usuariosService.usuarioActual.Cod_Usuario
            ).then((reservaciones) => {
  ...
  async rellenarArreglo(
    ...
    return this.syncreservacionesFiltrarFecha(
      Cod_Cancha,
  ...
  async TimeArrayFunction(
    ...
    for (
      var a = start == new Date().getHours() ? start + 1 : start;
      a < end;
      ++a
    ) {
      let id = a;
  ...
  async selectCategory() {
    switch (this.segment) {
      case 0:
        ...
        this.syncgGtReservacionesConfirmadas(
          this.usuariosService.usuarioActual.Cod_Usuario
        ).then((reservaciones) => {
      ...
      case 1:
        // recibidos
        this.syncgGtReservacionesRecibidas(
          this.usuariosService.usuarioActual.Cod_Usuario
        ).then((reservaciones) => {
          this.reservaciones = reservaciones;
          console.log('reservaciones', this.reservaciones);
      ...
      case 2:
        // enviados
        this.syncgGtReservacionesEnviadas(
          this.usuariosService.usuarioActual.Cod_Usuario
        ).then((reservaciones) => {
          this.reservaciones = reservaciones;
      ...
      case 3:
        //hisyotial
        this.syncgGtReservacionesHistorial(
          this.usuariosService.usuarioActual.Cod_Usuario
        ).then((reservaciones) => {
          this.reservaciones = reservaciones;
      ...
      case 4:
        this.syncGetReservacionesResvision(
          this.usuariosService.usuarioActual.Cod_Usuario
        ).then((reservaciones) => {
          this.reservaciones = reservaciones;
        ...
        break;
      case 5:
        this.syncGetReservacionesCanceladas(
          this.usuariosService.usuarioActual.Cod_Usuario
        ).then((reservaciones) => {
          this.reservaciones = reservaciones;
