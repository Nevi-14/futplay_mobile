...
export class CambiarContrasenaPage {
  ...
  async cambiarContrasena(fCambiarContrasena: NgForm) {
    ...
    if (!continuar) {
      return this.alertasService.message('FUTPLAY', this.translateService.instant('ALL_FIELDS_REQUIRED'));
    }

    let data = fCambiarContrasena.value;
    if (data.Contrasena.length < 8) {
      return this.alertasService.message('FUTPLAY', this.translateService.instant('MIN_PASSWORD_LENGTH'));
    }
