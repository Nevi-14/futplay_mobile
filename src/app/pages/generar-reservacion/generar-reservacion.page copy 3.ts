...
// FUNCIONES GENERALES
 ...
 horaInicio($event){
  ...
  if(new Date().getDate() === new Date().getDate()){
    ...
    dateToUse = this.calendar.currentDate;
    start = this.Hora_Inicio.hours+1;
  
  }
  this.gestionReservacionesService.syncreservacionesFiltrarFecha(this.cancha.Cod_Cancha ,this.gestionReservacionesService.formatoFecha(this.calendar.currentDate,'-')).then(resp =>{
